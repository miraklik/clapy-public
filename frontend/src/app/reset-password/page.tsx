...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:8080/reset-password/request', {
        method: 'POST',
      ...
      if (res.ok) {
        setSuccess(true);
        setTimeout(() => router.push('/reset-password/confirm'), 3000);
      } else {
                ...
                >
                  <div className="bg-green-500/10 border border-green-500/20 text-green-400 p-4 rounded-xl flex items-center gap-3 text-sm shadow-inner shadow-green-500/5">
                    ...
                    <div>
                      ...
                      <p className="text-xs opacity-90">Check your inbox for instructions.</p>
                    </div>
                  </div>
                </motion.div>
              )}
